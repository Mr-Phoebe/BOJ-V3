...
def mainPage(request):
    return render(request, 'newtpl/fresh/index.html', {})

def endedPage(request):
    return render(request, 'newtpl/fresh/ended.html', {})

def teamLogin(request):
    logger.info(unicode(request).replace('\n', '\t'))
    try:
        if request.method != 'POST':
            return render(request, 'newtpl/fresh/team_login.html', {})
...
def teamRegister(request):
    ...
    try:
        if request.method != 'POST':
            return render(request, 'newtpl/fresh/team_register.html', {})
...
def teamInfo(request, tid):
    ...
    try:
        ...
        return render(request, 'newtpl/fresh/team_info.html', {'target_team': target_team, 'members': members, 'full': full})
    except Exception as e:
        logger.error(unicode(e).replace('\n', '\t'))
        messages.add_message(request, messages.INFO, unicode(e))
...
def addTeamMember(request, tid):
    ...
    try:
        ...
        target_team = Team.getById(tid)
        if request.method != 'POST':
            return render(request, 'newtpl/fresh/add_member.html', {'target_team': target_team, 'gender_choices': Const.GENDER_CHOICES, 'school_choices': Const.SCHOOL_CHOICES, 'grade_choices': Const.GRADE_CHOICES})
...
def resetPasswd(request, tid):
    ...
    try:
        ...
        target_team = Team.getById(tid)
        if request.method != 'POST':
            return render(request, 'newtpl/fresh/reset_passwd.html', {'target_team': target_team})
...
def viewAllTeams(request):
    ...
    try:
        ...
        return render(request, 'newtpl/fresh/view_teams.html', {'teams': teams, 'pending': pending, 'accepted': accepted, 'failed': failed, 'skipped': skipped})
...
def viewAcceptedTeams(request):
    ...
    try:
        ...
        for team in ac_team_list:
            members = Contestant.getTeamContestant(team)
            teams.append({'info': team, 'members': members})
        return render(request, 'newtpl/fresh/view_accepted_teams.html', {'teams': teams})
...
def adminLogin(request):
    ...
    try:
        if request.method != 'POST':
            return render(request, 'newtpl/fresh/enter_admin.html', {})
...
def judgeTeam(request, tid = 1):
    ...
    try:
        ...
        skipped = team_list.filter(status = 'Skipped').count()
        members = Contestant.getTeamContestant(team)
        return render(request, 'newtpl/fresh/judge_team.html', {'team': team, 'members': members, 'pending': pending, 'accepted': accepted, 'failed': failed, 'skipped': skipped, 'admin': True})
