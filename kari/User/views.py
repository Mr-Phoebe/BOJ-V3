...
def addMultiUser(request):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            return render(request, 'newtpl/user/addmultiuser.html', {'univ_name':u.university.name, 'schools':schools, 'tpl':tpltrue})
        else:
...
def csvDownload(request, usname):
    response  = HttpResponse(mimetype='text/csv')
    response.write('\xEF\xBB\xBF')
    response['Content-Disposition'] = 'attachment; filename=result.csv'
    reader = csv.reader(file('/tmp/%s.csv'%usname))
...
def listStudent(request, page_id = '1'):
    logger.info(str(request).replace("\n","\t"))
    try:
        u = User.getSessionUser(request.session)
        if not u:
        ...
        paginator = Paginator(allstudent, Const.STUDENT_PER_PAGE)
        page_id = min(max(int(page_id), 1), paginator.num_pages)
        return render(request, 'newtpl/user/allstudent.html', {'allstudent':paginator.page(page_id), 'tpl':tpltrue})
...
def listStudent2(request, page_id='1'):
    logger.info(str(request).replace("\n","\t"))
    try:
        u = User.getSessionUser(request.session)
        if not u:
        ...
        if form.is_valid():
            ...
            target_user = User.getUserByRawUsername(username)
            if not target_user:
                raise Err(request, 'no such user')
            return redirect('User:viewuser', uid = target_user.uid)
        else:
            raise Err(request, 'no such user')
    except Exception as e:
        logger.error(str(e).replace("\n","\t"))
...
def manageGroup(request, pageId = '1'):
    logger.info(str(request).replace("\n","\t"))
    try:
        u = User.getSessionUser(request.session)
        if u == False:
        ...
        paginator = Paginator(group_list, Const.CLASS_PER_PAGE)
        pageId = min(max(int(pageId), 1), paginator.num_pages)

        return render(request, 'newtpl/user/managegroup.html', {'groups': paginator.page(pageId), 'tpl':{'sp':True}})
...
def ResetPasswd(request, uid):
    ...
    try:
        ...
        if request.method == 'GET':
            return render(request, 'newtpl/user/resetone.html', {'tpl':tpltrue, 'resetuser':utoreset})
        ...
        if passwd1 == passwd2:
            ...
            try:
                utoreset._vRawPasswd()
            except:
                raise Err(request, 'passwd illegal')
            utoreset._encPasswd()
            utoreset.save()
        else:
            raise Err(request, 'passwd diff')
...
def resetGroupMemberPasswd(request, group_id, mode='random'):
    ...
    try:
        ...
        suc = sorted(succ.iteritems(),key=lambda d:d[0])
        return render(request, 'newtpl/user/resetresult.html', {'group':g, 'succ':suc, 'tpl':tpltrue})
    except Exception as e:
        logger.error(str(e).replace("\n","\t"))
        return render(request, Err.ERROR_PAGE)
