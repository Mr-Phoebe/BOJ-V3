...
class addGeneralForm( forms.Form):
    """
    ...
    def __init__(self, langList, *args, **kwargs):
        super(addGeneralForm, self).__init__(*args, **kwargs)
...
class generalListForm( forms.Form):
    """
    form used to filter General Submission List 
    """
    ...
    problem_id = forms.IntegerField( 
            required=False, 
            label=u'题号',
            widget=forms.TextInput(attrs={'class':'input-medium', 'placeholder':u'题号'})
            )
    ...
    def __init__(self, langList, *args, **kwargs):
        super(generalListForm, self).__init__(*args, **kwargs)
